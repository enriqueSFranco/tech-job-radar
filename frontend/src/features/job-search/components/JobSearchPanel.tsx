...
import { clearSearch, setKeyword, setLocation } from "../search.slice";
...
export function AnimatedPanel({
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 z-50 bg-black backdrop-blur-xl flex flex-col items-center transition-all duration-300 ease-in-out",
...
export function SearchFormPanel({
  ...
  const currSearchRef = useRef({keyword: "", location: ""})
  const navigate = useNavigate();
  useBodyScrollLock(openPanel);
  ...
  function handleSubmit(e: React.FormEvent<SearchFormElements>) {
    ...
    if (isSameKeyword && isSameLocation) return
    ...
    const keywordSlug = encodeURIComponent(slugify(keyword))
  ...
  return (
    <div className="w-full">
      <Header
        leftSlot={<h2 className="text-2xl font-bold">buscar</h2>}
        rightSlot={
      ...
      <form
        ...
        className="w-full px-4 py-6 max-w-lg mx-auto flex flex-col gap-4"
      >
        <TextField
