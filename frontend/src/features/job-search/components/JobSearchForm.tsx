...
import { SearchFormElements } from "../interfaces";
import { selectFormSearch } from "../search.selectors";
import { setKeyword, setLocation } from "../search.slice";
import { AnimatedPanel, SearchFormPanel } from "./JobSearchPanel";
import { SearchButton } from "./SearchButton";
import { slugify } from "@/utils/slugify";

export function SearchForm() {
  const [showSearchFormPanel, updateSearchFormPanel] = useState(false);
  const currSearchRef = useRef<{ keyword: string; location: string }>({
    keyword: "",
    location: "",
  });
  ...
  function handleSubmit(e: React.FormEvent<SearchFormElements>) {
    ...
    const prevKeyword = currSearchRef.current.keyword;
    const prevLocation = currSearchRef.current.location;

    const newKeyword = keyword.trim();
    const newLocation = location.trim();

    const isSameKeyword = prevKeyword === newKeyword;
    const isSameLocation = prevLocation === newLocation;

    if (isSameKeyword && isSameLocation) {
      return;
    }

    currSearchRef.current = {
      keyword: newKeyword,
      location: newLocation,
    };

    console.log("realizando busqueda de emepleos");
    dispatch(fetchJobs({ keyword, location }));

    const keywordSlug = encodeURIComponent(slugify(newKeyword));
    const urlRedirect = `/empleos/${keywordSlug}`;
    navigate(urlRedirect);
    // clearSearch({ keyword, location });
  }

  function toggleSearchFormPanel() {
    updateSearchFormPanel((prevState) => !prevState);
  }

  if (isBreakpoint("mobile") || isBreakpoint("mobileLarge")) {
    return (
      <>
        ...
        {showSearchFormPanel && (
          <AnimatedPanel open={showSearchFormPanel}>
            <SearchFormPanel
              openPanel={showSearchFormPanel}
              onClose={toggleSearchFormPanel}
            />
          </AnimatedPanel>
        )}
      </>
  ...
  return (
    <form
      className="flex gap-4 justify-between w-2/3 h-14 mx-auto outline-[1px] outline-gray-400"
